...
export const fhirRoutes = new Elysia({ prefix: '/v/:fhir_release/fhir', tags: ['fhir'] })
    ...
    try {
        ...
        const body = text.replaceAll(config.fhir.serverBase, `${config.baseUrl}/v/${params.fhir_release}/fhir`);
        return body;
    }
    catch (error) {
        set.status = 500;
    ...
    try {
        ...
        const headers = new Headers();
        request.headers.forEach((v, k) => k !== 'host' && k !== 'connection' && headers.set(k, v));
        headers.set('accept', 'application/fhir+json');
        const resp = await fetch(target, {
