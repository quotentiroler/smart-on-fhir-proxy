...
class SmartConfigService {
    constructor() {
        ...
        this.cacheTTL = parseInt(process.env.SMART_CONFIG_CACHE_TTL || '300000'); // 5 minutes default
        this.keycloakDiscoveryUrl = `${config.keycloak.baseUrl}/realms/${config.keycloak.realm}/.well-known/openid-configuration`;
    }
    /**
     ...
     */
    async fetchAndBuildSmartConfig() {
        try {
     ...
     */
    buildSmartConfigFromOpenID(openidConfig) {
