...
describe('Keycloak Admin Integration', () => {
    ...
    test('should be able to list clients in the realm', async () => {
        try {
            ...
            const backendClient = clients.find((c) => c.clientId === 'smart-on-fhir-backend');
            if (backendClient) {
    ...
    test('should verify service account has admin role', async () => {
        try {
            ...
            if (!serviceAccountUser.id)
                throw new Error('Service account user ID is undefined');
            ...
            const hasAdminRole = roleMappings.some((role) => role.name === 'admin');
    ...
    test('should be able to create a test client', async () => {
        ...
        try {
            ...
            if (existingClients.length > 0) {
                ...
                if (!clientId)
                    throw new Error('Existing client ID is undefined');
            ...
            if (!deleteClientId)
                throw new Error('Verify client ID is undefined');
